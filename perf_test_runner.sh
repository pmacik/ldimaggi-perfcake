...
cp perfcake/$INPUT_FILE perfcake-7.4/resources/scenarios/
...
sed -e "s/THETOKEN/$token/g" $PERFCAKE_HOME/resources/scenarios/$INPUT_FILE > $PERFCAKE_HOME/resources/scenarios/output.xml

echo "======= echo the prefcake config file here ================"
cat $PERFCAKE_HOME/resources/scenarios/output-march7.xml

# Run the test - single token == single user */
$PERFCAKE_HOME/bin/perfcake.sh -s output.xml -Dthread.count=$THREAD_COUNT
